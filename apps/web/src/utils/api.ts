...
import { REST_AI_URL, RPC_ENDPOINT } from '@/contants/network';
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const externalFetch = (url: string, method: string, body = {}, isUpload = false): any => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const options: any = {
    url: `${RPC_ENDPOINT}${url}`,
    method,
    headers: isUpload ? uploadHeaders : headers,
  };
...
export const externalGet = (path: string) => externalFetch(path, 'GET');
export const externalPost = (path: string, body: object) => externalFetch(path, 'POST', body);
