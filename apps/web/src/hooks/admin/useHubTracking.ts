...
const useHubTracking = () => {
  ...
  const fetchTrachkingForChart = async () => {
    setLoading(true);
    try {
      const { data } = await instance.getExternal(`/api/admin/trackings/get-hub-transactions?startDate=${convertDateToTracking(startDate)}&endDate=${convertDateToTracking(endDate)}`);
  ...
  const fetchTrackingSumrary = async () => {
    ...
    try {
      const { data } = await instance.getExternal(`/api/admin/trackings/get-hub-summary?startDate=${convertDateToTracking(startDate)}&endDate=${convertDateToTracking(endDate)}`);
