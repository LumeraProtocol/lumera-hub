...
const supernodeItemSchema = z.object({
  supernode_account: z
    ...
    .regex(/^lumera1[a-z0-9]{38,39}$/, 'Supernode account must start with "lumera1" and be a valid Lumera address (38-39 characters)'),

  validator_address: z
    .string()
    .min(1, 'Validator address cannot be empty')
    .regex(/^lumeravaloper1[a-z0-9]{38,39}$/, 'Validator address must start with "lumeravaloper1" and be a valid Lumera address (38-39 characters)'),
  ...
  ip_address: z
    ...
    .max(50, 'IP address is too long (maximum 50 characters)'),
});
