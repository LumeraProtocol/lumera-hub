...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      message['@type'].indexOf('action.v1.MsgRequestAction') === -1 ||
      message?.actionType !== 'cascade' ||
      message?.creator !== user.lumeraAddress
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    const fileSizeKbs = Number(message?.fileSizeKbs);
    const configFileSizeKbs = Number(config.firstUploadCascade.size);

    switch (config.firstUploadCascade.condition) {
      case '>':
        if (fileSizeKbs <= configFileSizeKbs) {
          return NextResponse.json(
            {
              success: false,
              error: `The size is less the minimum(${formatNumber(configFileSizeKbs, { decimalsLength: 0 })} Kb) requirement.`,
            },
            { status: 400 }
          );
        }
      ...
      case '<=':
        if (fileSizeKbs > configFileSizeKbs) {
          return NextResponse.json(
            {
              success: false,
              error: `The size is greater than the maximum(${formatNumber(configFileSizeKbs, { decimalsLength: 0 })} Kb) requirement.`,
            },
            { status: 400 }
          );
        }
      ...
      case '<':
        if (fileSizeKbs >= configFileSizeKbs) {
          return NextResponse.json(
            {
              success: false,
              error: `The size is greater the maximum(${formatNumber(configFileSizeKbs, { decimalsLength: 0 })} Kb) requirement.`,
            },
            { status: 400 }
          );
        }
      ...
      case '=':
        if (fileSizeKbs !== configFileSizeKbs) {
          return NextResponse.json(
            {
              success: false,
              error: `The size does not match the required.`,
            },
            { status: 400 }
          );
        }
      ...
      default:
        if (fileSizeKbs < configFileSizeKbs) {
          return NextResponse.json(
            {
              success: false,
              error: `The size is less than the minimum(${formatNumber(configFileSizeKbs, { decimalsLength: 0 })} Kb) requirement.`,
            },
            { status: 400 }
          );
        }
